...
func TestValidateDownloadPath(t *testing.T) {
	...
	if err := os.WriteFile(existingFile, []byte("test"), PermStandardFile); err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateDownloadPath(tt.path, false, PermStandardDir) // allowOverwrite = false, dirPermissions = 0755
...
func TestSecureCreateFile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			file, err := SecureCreateFile(tt.path, false, PermStandardFile, PermStandardDir) // allowOverwrite = false, filePermissions = 0644, dirPermissions = 0755
			...
			if tt.wantErr {
				...
				if err != nil {
					...
					mode := info.Mode()
					expected := os.FileMode(PermStandardFile) // From test parameters
